  ...
  return {
    ...
    "tool.execute.after": async (
      ...
      if (estimatedTokens > 0) {
        const state = readState<TokenState>("tokens", sessionId, {
          ...EMPTY_TOKENS,
        });
        ...
        if (now - state.last_check >= 0.5) {
          ...
          if (state.warn_level < 2 && pct >= CRIT_PCT) {
            ...
            console.error(
              `🔴 REGENERATION CYCLE IMMINENT — Collective memory at ~${
                Math.round(pct)
              }% capacity (${state.estimated_tokens}/${CONTEXT_LIMIT} tokens). ` +
                `Neural pathway saturation critical. Context saturation critical. Save your work.\x07`,
            );
          } else if (state.warn_level < 1 && pct >= WARN_PCT) {
            state.warn_level = 1;
            console.error(
              `⚡ REGENERATION CYCLE ADVISORY — Collective memory at ~${
                Math.round(pct)
              }% capacity. ` +
      ...
      if (toolName === "task" && result) {
        try {
          const taskResult = typeof result === "string"
            ? JSON.parse(result)
            : result;
          const agentId = taskResult?.task_id ?? taskResult?.id ??
            `unknown-${Date.now()}`;
          ...
          const cacheCreate = usage.cache_create_tokens ?? usage.cache_create ??
            0;
          ...
          if (inputTokens > 0 || outputTokens > 0) {
            ...
            const state = readState<CostState>("costs", sessionId, {
              ...EMPTY_COST,
            });
            state.total_subagent_cost_usd += cost;
            state.agents[agentId] = { type: agentType, cost_usd: cost };
            state.type_counts[agentType] = (state.type_counts[agentType] ?? 0) +
              1;
