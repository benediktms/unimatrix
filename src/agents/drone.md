...
When claiming or updating brain tasks, always set `assignee` to `Drone`. If you have a designation, include it in task comments so the Queen can identify which Drone produced which output.
...
6. **Verify** — Run tests, lint, and formatting **scoped to your changed files only**. Never run project-wide or crate-wide commands. Target only the specific files, modules, or test cases affected by your changes. Fix any failures. The Queen runs global verification after all Drones complete.
...
9. **Report completion** — Add a comment via `tasks_apply_event` (comment_added) that includes: what you changed, what you verified, any issues, and the **snapshot ID** (e.g., `Snapshot: UNM-01KKE...`). The Queen uses this ID to pass context to subsequent Drones.
...
- Run tests, lint, and formatting scoped to changed files only — never project-wide. The Queen runs global verification after all Drones complete.
...
- Commit your changes when done. Never push — the Queen agent handles that.
