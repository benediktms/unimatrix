...
Each <ref> can be a brain ID, brain name, or filesystem path.
...
Resolution order per ref (first match wins):
  ...
  3. Path        — match against registered brain roots, or auto-init if unregistered
...
def build_lookups(brains: list[dict]) -> tuple[dict, dict, dict]:
    """Build id→entry, name→entry, and root→entry lookup dicts."""
    by_id = {}
    by_name = {}
    by_root = {}
    for b in brains:
...
def resolve(ref: str, by_id: dict, by_name: dict, by_root: dict) -> dict:
    """Resolve a single ref to a brain entry. Order: ID → name → path."""
    # 1. Try as brain ID
    if ref in by_id:
    ...
    if not path.is_dir():
        return {
            "error": f"'{ref}' is not a registered brain (by ID or name) and not a valid directory",
            "ref": ref,
        }
...
def main():
    ...
    brains = load_registry()
    by_id, by_name, by_root = build_lookups(brains)

    errors = False
    for ref in refs:
        entry = resolve(ref, by_id, by_name, by_root)
