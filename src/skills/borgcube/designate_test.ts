 ...
 * - Shared ordinals: batch roles (Drone, Probe, Subroutine) share one ordinal
 * - Unique ordinals: Cortex/Vinculum get ordinal == their position
...
Deno.test("N=1: returns exactly 1 designation", () => {
  const { designations } = designate(1, "Drone");
...
Deno.test("N=5: returns exactly 5 designations", () => {
  const { designations } = designate(5, "Drone");
...
Deno.test("N=12: returns exactly 12 designations", () => {
  const { designations } = designate(12, "Drone");
...
const ROLES: Role[] = ["Drone", "Vinculum", "Probe", "Cortex", "Subroutine"];
...
Deno.test("trimatrix=true: designation contains 'Trimatrix' + number", () => {
  const { designations, trimatrix_id } = designate(3, "Drone", true);
...
Deno.test("trimatrix=true: trimatrix_id is between 1 and 999", () => {
  for (let i = 0; i < 20; i++) {
    const { trimatrix_id } = designate(1, "Drone", true);
...
Deno.test("trimatrix=false: designation contains 'Unimatrix Zero'", () => {
  const { designations, trimatrix_id } = designate(3, "Drone", false);
...
Deno.test("trimatrix omitted: designation contains 'Unimatrix Zero'", () => {
  const { designations, trimatrix_id } = designate(3, "Drone");
  assertEquals(trimatrix_id, undefined);
  for (const d of designations) {
    assertMatch(d, /Unimatrix Zero$/);
  }
...
const SHARED_ORDINAL_ROLES: Role[] = ["Drone", "Probe", "Subroutine"];
...
for (const role of ["Cortex", "Vinculum"] as Role[]) {
...
Deno.test("UNIQUE_ORDINAL_ROLES contains Cortex and Vinculum", () => {
  assertEquals(UNIQUE_ORDINAL_ROLES.has("Cortex"), true);
  assertEquals(UNIQUE_ORDINAL_ROLES.has("Vinculum"), true);
  assertEquals(UNIQUE_ORDINAL_ROLES.has("Drone"), false);
  assertEquals(UNIQUE_ORDINAL_ROLES.has("Probe"), false);
  assertEquals(UNIQUE_ORDINAL_ROLES.has("Subroutine"), false);
});
