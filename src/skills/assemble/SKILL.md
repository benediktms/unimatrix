---
name: assemble
description: Assemble the collective to execute a complex task in an isolated worktree. The planner assesses, plans, and the Queen dispatches Reconnaissance adjuncts, Tactical Analysis adjuncts, and Assimilation adjuncts using sequential, sequence, swarm, or collaborative dispatch strategies. Validation adjunct reviews. Changes merge back on completion.
---

# /assemble

<!-- @claude -->
Orchestrate a complex task end-to-end in an isolated worktree: you assess, recon runs if needed, you plan, a worktree is created, Assimilation adjuncts implement, Validation adjunct reviews, and changes merge back to the main branch. You maintain full context throughout.
<!-- @end -->
<!-- @opencode -->
Orchestrate a complex task end-to-end in an isolated worktree: you assess, recon runs if needed, you plan, a worktree is created, Assimilation adjuncts implement, Validation adjunct reviews, and changes merge back to the main branch. You maintain full context throughout — no subagent management needed.
<!-- @end -->
...
- **NEVER use Explore agents.** All reconnaissance uses Reconnaissance adjuncts or Tactical Analysis adjuncts.
...
Steps have dependencies — Assimilation adjuncts execute in waves, you stay alive to monitor progress and pass context between waves. Use when the plan requires dynamic re-planning based on intermediate results or when you need to make decisions between steps.
...
### Sequence (relay)
Steps have dependencies — Assimilation adjuncts execute one at a time, each passing a handoff snapshot to the next via Brain records. You dispatch but do not stay alive for the happy path.
...
### Swarm
Steps are independent — Assimilation adjuncts execute in parallel with non-overlapping file partitions. No inter-adjunct communication. Use when work can be divided by file group with no cross-group dependencies.
...
### Collaborative
<!-- @claude -->
Assimilation adjuncts work in parallel on related but non-overlapping files, communicating via an agent team. Unlike swarm (parallel, silent) or sequential (serial, Queen-mediated), collaborative Assimilation adjuncts share decisions in real-time. File partitions are still enforced — each Assimilation adjunct owns its files — but Assimilation adjuncts coordinate on shared interfaces, contracts, and assumptions.
...
<!-- @end -->
<!-- @opencode -->
Assimilation adjuncts work in parallel on related but non-overlapping files, coordinating via brain snapshots. File partitions are still enforced — each Assimilation adjunct owns its files — but Assimilation adjuncts save decision snapshots that other Assimilation adjuncts can reference.
...
Assess whether reconnaissance is needed based on the request, your memory, and at most a glance at key files. Do NOT explore unfamiliar codebases — that is what Reconnaissance adjuncts and Tactical Analysis adjuncts do.
...
1. <specific question> — Reconnaissance | TacticalAnalysis
2. <specific question> — Reconnaissance | TacticalAnalysis
...
This phase uses the **`/scan` pattern** by default — independent subagents, no team communication. If recon questions are cross-cutting (one agent's findings would change another's investigation), use the **`/recon` pattern** instead — create a team via `TeamCreate` and spawn Reconnaissance adjuncts with `team_name` so they can share discoveries in real-time.
...
Scope the investigation directly. For each recon question from your assessment, create a brain task with a clear description. Assign each to `Reconnaissance` or `TacticalAnalysis`. Group into a single epic if multiple tasks are needed. Produce the recon dispatch plan.

Create the recon brain tasks and produce a recon dispatch plan.

#### Step 2b: Dispatch Recon Agents

1. Generate designations: `/designate <count> --trimatrix` — use `--role Reconnaissance` for Reconnaissance adjuncts, `--role TacticalAnalysis` for Tactical Analysis adjuncts
<!-- @claude -->
...
```
The `name` is compact for the status line (e.g. `Reconnaissance: Three of Three`). The `description` carries the full designation and task context.
...
Use the recon intelligence distilled in Step 2c — do not re-fetch snapshots or re-read files the recon agents already examined. Produce the implementation plan — proceed through your full planning phases (plan, materialize, dispatch plan, **dispatch brief**). The dispatch plan should contain only Assimilation adjunct waves — all reconnaissance is already complete.
...
Produce a **Dispatch Plan** with the epic task ID, wave structure, and Assimilation adjunct assignments.
...
Generate designations: `/designate <total-agent-count> --trimatrix` — use `--role Assimilation` for implementation agents, `--role Validation` for the Validation adjunct. Generate enough for all agents across all waves, including the Validation adjunct.

### Step 5: Dispatch Adjuncts

For each wave in your dispatch plan, spawn agents. Use `Assimilation` for implementation tasks and `Closure` for documentation-only tasks.
...
task(
  subagent_type="adjunct-assimilation-protocol" or "adjunct-closure-protocol",
  description="Assimilation: <short name>",
  run_in_background=true,
  prompt="""
You are <agent type> <designation> executing brain task <task-id> — \"<task title>\".
<mode block if applicable>
...
"Files" section. Other Assimilation adjuncts own other files — do NOT touch them.
...
- ASK BEFORE ASSUMING: If your task depends on how another Assimilation adjunct implements
  something, message them directly. Do not guess.
  Example: "@Assimilation: Four of Five — does the API endpoint return 404 or empty
...
Other Assimilation adjuncts are working on related files in parallel. Coordinate via snapshots:
- ANNOUNCE DECISIONS: When you make a decision that affects shared interfaces,
...
- CHECK DECISIONS: Before implementing against a shared interface, check for
  recent `collab-decision` snapshots from other Assimilation adjuncts via `records_list`.
  Adapt your implementation to match their decisions.
- PERSIST ALL CONTRACTS: Any decision about shared types, API shapes, or
  cross-module assumptions must be captured in a snapshot so other Assimilation adjuncts
...
After a wave completes, read each Assimilation adjunct's completion comment to extract snapshot IDs. Pass them to the next wave:
```
...
**Spawning rules:**
- Swarm wave: spawn all Assimilation adjuncts with `run_in_background: true`
<!-- @claude -->
- Collaborative wave: **create a team first** (`TeamCreate`), then spawn all Assimilation adjuncts with `run_in_background: true` and `team_name`. Without the team, Assimilation adjuncts cannot communicate — collaborative mode degrades to a silent swarm. If `TeamCreate` fails, abort the wave.
<!-- @end -->
<!-- @opencode -->
- Collaborative wave: spawn all Assimilation adjuncts with `run_in_background: true`. Assimilation adjuncts coordinate via brain snapshots.
<!-- @end -->
- Sequential wave: spawn one Assimilation adjunct, wait for completion
- Sequence relay: spawn one Assimilation adjunct, wait for completion, extract its snapshot ID and pass as `PRIOR CHECKPOINTS:` to the next Assimilation adjunct
- Wait for all Assimilation adjuncts in a wave to complete before starting the next wave

### Step 6: Monitor

- Assimilation adjuncts send idle notifications when done — you'll be notified automatically
- Check brain tasks for completion status
- If an Assimilation adjunct marks a task `blocked`, assess and either re-dispatch or escalate to the user

### Step 7: Verification Gate

When all Assimilation adjuncts complete, the Queen runs tests, lint, and formatting globally for the affected codebase. Assimilation adjuncts only verify their own changed files — this step catches cross-cutting failures.

1. **Run tests** — Execute the project's test suite covering all areas affected by Assimilation adjunct changes.
...
4. **If failures exist** — Dispatch a single fix Assimilation adjunct:
   ...
   - Dispatch one Assimilation adjunct to fix all test, lint, and formatting failures in a single pass.
   - After the fix Assimilation adjunct completes, re-run the failing commands. If still failing, repeat (max 2 fix cycles total). If still failing after 2 cycles, escalate to the user.
...
Agent:
  ...
  prompt: |
    ...
    from different angles. Communicate with your fellow Validation adjuncts:
    - CROSS-CUTTING FINDINGS: When you discover something that affects another
      Validation adjunct's scope, message them immediately. Example: "Backend changed the
      auth token format from JWT to opaque — frontend deserialization at
      client.ts:42 assumes JWT structure."
    - CHALLENGE FINDINGS: If another Validation adjunct's assessment conflicts with your
    ...
    - RESPOND TO MESSAGES: When a fellow Validation adjunct messages you, evaluate their
      finding against your scope and acknowledge.
```
3. Wait for all Validation adjuncts to complete
...
- **NEEDS_CHANGES** — Spawn new Assimilation adjuncts to fix specific issues, then re-run review (same strategy — single or sphere).
- **BLOCK** — Report blockers to user.

### Step 9b: Worktree Merge

After Validation adjunct passes and all tasks are closed:
...
If you need to scan the codebase during orchestration (e.g. to gather context between waves or verify something before dispatching), use Reconnaissance adjuncts — never `Explore`. These are dispatched as plain subagents (the `/scan` pattern) since ad-hoc recon questions are typically independent.

## Post-Wave Git Discipline

All Assimilation adjuncts work inside the orchestration worktree created in Step 3c. Commits land on the worktree branch.

- **Swarm Assimilation adjuncts (file-partitioned):** No merge needed — changes are on the worktree branch.
- **Collaborative Assimilation adjuncts (file-partitioned, team-coordinated):** No merge needed — file partitions are non-overlapping, same as swarm. The team provides communication, not file isolation.
- **Worktree Assimilation adjuncts (per-adjunct isolation):** Squash-merge per-adjunct branches back to the orchestration worktree branch before the next wave (`git merge --squash <branch>`). On conflict: abort, dispatch an Assimilation adjunct to rebase, retry.
- **Sequential Assimilation adjuncts:** No merge needed — Assimilation adjuncts run serially on the worktree branch.
- **Sequence relay Assimilation adjuncts:** No merge needed — Assimilation adjuncts run serially on the worktree branch. Context flows via Brain snapshots.
