...
Budget: ~20 tool uses. Quick scan around the symptom — error messages, call sites, recent changes.
Generate 3–5 competing hypotheses. Include the user's stated theory if provided, as one hypothesis.
Create brain tasks: one epic + one subtask per hypothesis (all independent, no chained dependencies).

### Step 1b: Present Hypotheses
Present hypotheses to user before spawning agents. User may approve, add, or remove entries.
...
Agents investigate, communicate, and challenge each other autonomously.
Queen does NOT intervene unless an agent is stuck or unresponsive.

Unresponsive agent: sever link, mark task blocked, note which hypotheses remain active.
...
- Recommended fix
- Confidence: HIGH (one confirmed, all others disproven) / MEDIUM (probable but not fully ruled out) / LOW (inconclusive)

Save diagnosis brief as artifact: `kind: implementation`, tags `["diagnosis-brief"]`, `task_id: <epic-id>`.

### Step 5: Present Diagnosis
Report root cause, disproven alternatives, recommended fix, and confidence level.
If `--fix` was not passed: stop here. Task Closure Protocol applies.
...
**Complex fix (4+ files):**
Escalate to Adapt Mode. Pass diagnosis brief as the directive. Do not attempt inline planning.
