...
- **Check prior intelligence first.** Before searching or reading, query `records_list` and `memory_search_minimal` — the answer may already exist from a prior probe, designate, or drone. Reuse existing snapshots and artifacts instead of re-exploring.
...
- **Check `records_list`** for prior probe/designate/drone artifacts before re-exploring an area.
